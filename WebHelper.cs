...
namespace WebHelper {
    ...
    public static class WebHelpers
    {
        ...
        public static HttpWebResponse MultipartFormDataPost(string postUrl, string userAgent, Dictionary<string, object> postParameters, 
            string formDataBoundary = "-----------------------------28947758029299", CookieContainer cookie = null)
        {
            string contentType = "multipart/form-data; boundary=" + formDataBoundary;

            byte[] formData = WebHelpers.GetMultipartFormData(postParameters, formDataBoundary);

            return WebHelpers.PostForm(postUrl, userAgent, contentType, formData, cookie);
        }
        ...
        private static byte[] GetMultipartFormData(Dictionary<string, object> postParameters, string boundary)
        {
            ...
            foreach (var param in postParameters)
            {
                ...
                else if (param.Value is FileItem)
                {
                    FileItem i = (FileItem)param.Value;

                    // Add just the first part of this param, since we will write the file data directly to the Stream
                    string header = string.Format(
                        "--{0}\r\nContent-Disposition: form-data; name=\"{1}\"; filename=\"{2}\";\r\nContent-Type: application/octet-stream\r\n\r\n",
                        boundary, param.Key, i.filename);
                    byte[] headerBytes = encoding.GetBytes(header);
                    formDataStream.Write(headerBytes, 0, headerBytes.Length);

                    // Write the file data directly to the Stream, rather than serializing it to a string.  This 
                    formDataStream.Write(i.data, 0, i.data.Length);
                    formDataStream.Write(encoding.GetBytes("\r\n"), 0, 2);
                }
                else
                {
        ...
        public struct FileItem
        {
            ...
            public FileItem(string filename, byte[] data)
            {
